...
void cloud_callback (const sensor_msgs::PointCloud2 &cloud_msg) {
     ...
     //pass.setFilterLimitsNegative (true);
     pass.filter (*cloud_passthrough_filtered);
     writer.write<pcl::PointXYZ> ("filteredz.pcd", *cloud_passthrough_filtered, false);

     pcl::PointXYZ minTotal, maxTotal;
     pcl::getMinMax3D (*cloud_passthrough_filtered, minTotal, maxTotal);
     std::cout << "Min: " << minTotal.y << std::endl;
     std::cout << "Max: " << maxTotal.y << std::endl;
    // Passthrough to remove table
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_table_filtered (new pcl::PointCloud<pcl::PointXYZ>);
    // pcl::PassThrough<pcl::PointXYZ> pass;
    pass.setInputCloud(cloud_passthrough_filtered);
    pass.setFilterFieldName ("y");
    pass.setFilterLimits (minTotal.y, minTotal.y + 0.15);
    // pass.setFilterLimitsNegative (true);
    pass.filter (*cloud_table_filtered);
    writer.write<pcl::PointXYZ> ("filteredy.pcd", *cloud_table_filtered, false);
     ...
     pcl::PointCloud<pcl::PointXYZ>::Ptr transformed1_cloud (new pcl::PointCloud<pcl::PointXYZ> ());
     pcl::PointCloud<pcl::PointXYZ>::Ptr transformed2_cloud (new pcl::PointCloud<pcl::PointXYZ> ());
     pcl::transformPointCloud (*cloud_table_filtered, *transformed1_cloud, transform_z);
     pcl::transformPointCloud (*transformed1_cloud, *transformed2_cloud, transform_y);

     writer.write<pcl::PointXYZ> ("rotXZ.pcd", *transformed2_cloud, false);



     // Creating the KdTree object for the search method of the extraction
     pcl::search::KdTree<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ>);
     tree->setInputCloud (transformed2_cloud);

     std::vector<pcl::PointIndices> cluster_indices;
     pcl::EuclideanClusterExtraction<pcl::PointXYZ> ec;
     ec.setClusterTolerance (0.02); // original = 2cm (0.02)
     ec.setMinClusterSize (10); //original = 100
     ec.setMaxClusterSize (25000);
     ec.setSearchMethod (tree);
     ec.setInputCloud (transformed2_cloud);
     ec.extract (cluster_indices);
     ...
     for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
     {
       ...
       pcl::PointCloud<pcl::PointXYZ>::Ptr cluster_filtered (new pcl::PointCloud<pcl::PointXYZ>);
       for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit)
         cloud_cluster->points.push_back (transformed2_cloud->points[*pit]); //*
       cloud_cluster->width = cloud_cluster->points.size ();
       cloud_cluster->height = 1;
       cloud_cluster->is_dense = true;
